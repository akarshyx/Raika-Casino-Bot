...
`bot.env.example`. `NOWPAYMENTS_API_KEY` is used to create payment
addresses/QR codes. Deposit settlement is performed by the independent
blockchain address monitor; NOWPayments IPN callbacks are intentionally
ignored for crediting. `SERVER_URL` is only needed by separately enabled
public callback features.
